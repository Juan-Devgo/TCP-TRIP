...
export default function HexEditor({ field, onChange }: EditorProps) {
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <label className="flex items-center gap-1.5 cursor-pointer select-none">
          ...
          />
          <span className="relative w-9 h-5 rounded-full bg-zinc-700 border border-zinc-600 inline-flex items-center transition-colors peer-checked:bg-violet-700 peer-checked:border-violet-600">
        ...
        {fixed ? (
          <div className="flex items-center gap-1.5">
            <input
              ...
              className={`${inputCls} w-16 text-violet-400 text-center`}
            />
      ...
      <InfoNote>
        El valor hexadecimal (checksum, ID, etc.) se introduce al crear el encabezado.
      </InfoNote>
    </div>
  );
}
